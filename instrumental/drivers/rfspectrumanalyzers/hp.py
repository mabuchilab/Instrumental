...
"""
Driver module for HP/Agilent 4000-series spectrum analyzer.
...
import numpy as np
from pint import UndefinedUnitError
from . import RFSpectrumAnalyzer
...
class HP4395(RFSpectrumAnalyzer, VisaMixin):
    """
    ...
    def set_center_frequency(self,cf):
        cf_Hz = cf.to(u.Hz).magnitude
        self.write('CENT {:10.0f}'.format(cf_Hz))

    def get_center_frequency(self):
        cf = (np.float(self.query('CENT?')) * u.Hz).to(u.MHz)
        return cf

    def set_frequency_span(self,sp):
        sp_Hz = sp.to(u.Hz).magnitude
        self.write('SPAN {:10.0f}'.format(sp_Hz))

    def get_frequency_span(self):
        sp = (np.float(self.query('SPAN?')) * u.Hz).to(u.MHz)
        return sp

    def set_resolution_bandwidth(self,rb):
        rb_Hz = rb.to(u.Hz).magnitude
        self.write('BW {:10.0f}'.format(rb_Hz))

    def get_resolution_bandwidth(self):
        rb = (np.float(self.query('BW?')) * u.Hz).to(u.MHz)
        return rb
    ...
    def get_spectrum(self):
        ...
        tmo = self.resource.timeout
        self.resource.timeout = 10000
        amp_dBm = np.fromstring(self.query('OUTPDTRC?'),sep=',')
        self.resource.timeout = tmo
        f_start = cf - span / 2.0
        f_stop = cf + span / 2.0
        n_pts = len(amp_dBm)
        f = Q_(np.linspace(f_start.magnitude,f_stop.magnitude,n_pts))
        return f, amp_dBm
